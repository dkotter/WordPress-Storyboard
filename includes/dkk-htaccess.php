...
if (stristr($_SERVER['SERVER_SOFTWARE'], 'apache') !== false) {
	function roots_htaccess_writable() {
		if (!is_writable(get_home_path() . '.htaccess')) {
			add_action('admin_notices', create_function('', "echo '<div class=\"error\"><p>" . sprintf(__('Please make sure your <a href="%s">.htaccess</a> file is writeable ', 'storyboard'), admin_url('options-permalink.php')) . "</p></div>';"));
		};
	}
	...
	function roots_add_h5bp_htaccess($rules) {
		...
		$filename = WP_CONTENT_DIR . '/themes/' . $theme_name . '/includes/h5bp-htaccess';
